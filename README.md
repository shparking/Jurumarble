...
- 대기실(만 편집 가능): 방장이 ▲▼로 순서 지정(안 정하면 랜덤), ✕로 강퇴(다시 입장 불가), 칸을 탭해 내용 편집(출발·세계여행·이동 칸은 고정, Enter로 줄바꿈, 내용에 맞는 이모지 자동). **내용을 바꾼 칸은 원래 기능이 사라지고 '수행 완료'만 있는 일반 칸이 됨** (편집창의 '원래대로'로 복구). 2명 이상이면 **게임 시작**
- 게임: 내 차례에만 굴리기 활성. 모든 폰에서 같은 이동 애니메이션 → 도착 칸 카드가 전원에게 표시, 행동 주체만 버튼 사용
  ...
  - 게임 선택권(10번 칸): 밸런스 / 라이어 / 훈민정음 중 하나를 골라 진행
