...
- 게임: 내 차례에만 굴리기 활성. 모든 폰에서 같은 이동 애니메이션 → 도착 칸 카드가 전원에게 표시, 행동 주체만 버튼 사용
  ...
  - 라이어 게임(13번 칸, 워드 울프 방식): 행동자가 카테고리(음식/영화/나라/기업·브랜드/스포츠/전자기기·앱/사람) 선택 → 전원에게 키워드 배정(한 명만 비슷한 다른 키워드, 본인도 모름) → 각자 폰에서 탭해 확인 → 순서대로 설명 → 투표 → 결과(라이어를 맞히면 라이어가 마심, 못 맞히면 라이어 빼고 다 마심)
