...
- 대기실(만 편집 가능): 방장이 ▲▼로 순서 지정(안 정하면 랜덤), 칸을 탭해 내용 편집(출발·세계여행·이동 칸은 고정, Enter로 줄바꿈, 내용에 맞는 이모지 자동). **내용을 바꾼 칸은 원래 기능이 사라지고 '수행 완료'만 있는 일반 칸이 됨** (편집창의 '원래대로'로 복구). 2명 이상이면 **게임 시작**
- 게임: 내 차례에만 굴리기 활성. 모든 폰에서 같은 이동 애니메이션 → 도착 칸 카드가 전원에게 표시, 행동 주체만 버튼 사용
  ...
  - 라이어 게임(13번 칸, 워드 울프 방식): 행동자가 카테고리(음식/영화/나라/기업·브랜드/스포츠/전자기기·앱/사람) 선택 → 전원에게 키워드 배정(한 명만 비슷한 다른 키워드, 본인도 모름) → 각자 폰에서 탭해 확인 → 순서대로 설명 → 투표 → 결과 공개(누가 다른 키워드였는지, 두 키워드, 득표 수만 보여주고 승패 판정은 없음)
