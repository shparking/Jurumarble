...
- 게임: 내 차례에만 굴리기 활성. 모든 폰에서 같은 이동 애니메이션 → 도착 칸 카드가 전원에게 표시, 행동 주체만 버튼 사용
  - 놉카드 칸: 자동 지급 / 일반 칸: 수행 완료. 놉카드는 **순서 목록의 내 배지를 눌러** 사용 또는 다른 사람에게 양도(본인만, 확인 후 전원에게 알림)
  ...
  - **돌발 미션**: 주사위를 굴릴 때마다 10% 확률로 한 명에게만 몰래 미션(3분/5분). 전원 화면에 팝업(수행자는 미션 내용, 나머지는 안내)이 뜨고 각자 '확인'을 눌러 누가 인지했는지 표시됨. 시간이 끝나면 수행자 공개 → 5지선다로 미션 맞히기 → 3·2·1 → 결과. 절반 이상이 맞히면 수행자가, 못 맞히면 틀린 사람들이 마심
